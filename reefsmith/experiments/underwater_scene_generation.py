...
from reefsmith.agent_utils.articulated_retrieval_server import (
    ArticulatedRetrievalServer,
)
from reefsmith.agent_utils.geometry_generation_server import GeometryGenerationServer
from reefsmith.agent_utils.house import HouseLayout, HouseScene, RoomGeometry
from reefsmith.agent_utils.hssd_retrieval_server import HssdRetrievalServer
from reefsmith.agent_utils.materials_retrieval_server import MaterialsRetrievalServer
from reefsmith.agent_utils.objaverse_retrieval_server import ObjaverseRetrievalServer
...
from reefsmith.experiments.base_experiment import BaseExperiment
...
from reefsmith.micro_habitat_agents.stateful_micro_habitat_agent import (
    StatefulMicroHabitatAgent,
)
from reefsmith.utils.logging import ConsoleLogger, FileLoggingContext
from reefsmith.utils.parallel import run_parallel_isolated
from reefsmith.utils.print_utils import bold_green, yellow
...
def _export_scene_blend_file(
    ...
    """
    from reefsmith.agent_utils.rendering import save_scene_as_blend
...
def _generate_room(
    ...
    if start_idx <= 0:  # Run furniture if starting from furniture or earlier.
        with custom_span("furniture_placement"):
            ...
            furniture_agent = BaseExperiment.build_furniture_agent(
                cfg_dict=cfg_dict,
                compatible_agents=(
                    UnderwaterSceneGenerationExperiment.compatible_furniture_agents
                ),
                logger=logger,
    ...
    if start_idx <= 1:  # Run wall_objects if starting from wall_objects or earlier.
        with custom_span("wall_object_placement"):
            ...
            wall_agent = BaseExperiment.build_wall_agent(
                cfg_dict=cfg_dict,
                compatible_agents=UnderwaterSceneGenerationExperiment.compatible_wall_agents,
                logger=logger,
    ...
    if start_idx <= 2:  # Run ceiling if starting from ceiling or earlier.
        with custom_span("ceiling_object_placement"):
            ...
            ceiling_agent = BaseExperiment.build_ceiling_agent(
                cfg_dict=cfg_dict,
                compatible_agents=(
                    UnderwaterSceneGenerationExperiment.compatible_ceiling_agents
                ),
                logger=logger,
                ceiling_height=room_geometry.wall_height,
                render_gpu_id=render_gpu_id,
    ...
    with custom_span("manipuland_placement"):
        ...
        manipuland_agent = BaseExperiment.build_manipuland_agent(
            cfg_dict=cfg_dict,
            compatible_agents=(
                UnderwaterSceneGenerationExperiment.compatible_manipuland_agents
            ),
            logger=logger,
            render_gpu_id=render_gpu_id,
        )
        ...
        with trace(workflow_name="floor_plan_generation", metadata=trace_metadata):
            with custom_span("floor_plan_generation"):
                floor_plan_agent = BaseExperiment.build_floor_plan_agent(
                    cfg_dict=cfg_dict,
                    compatible_agents=(
                        UnderwaterSceneGenerationExperiment.compatible_floor_plan_agents
                    ),
                    logger=logger,
                    render_gpu_id=render_gpu_id,
                )
...
class UnderwaterSceneGenerationExperiment(BaseExperiment):
    """An experiment that generates underwater coral reef environments.

    Despite the indoor-derived internal vocabulary retained for serialization
    compatibility (rooms, walls, floor plan, furniture, manipulands), this
    experiment runs the ReefSmith ecological pipeline:
    seabed topography -> structural reef -> reef accents -> micro-habitat.
    """

    compatible_floor_plan_agents = {
        "stateful_floor_plan_agent": StatefulSeabedAgent,
    }
    compatible_furniture_agents = {
        "stateful_furniture_agent": StatefulReefAgent,
    }
    compatible_manipuland_agents = {
        "stateful_manipuland_agent": StatefulMicroHabitatAgent,
    }
    ...
    def _run_parallel_generation(
        ...
        for scene_id, prompt in prompts_with_ids:
            ...
            tasks.append(
                (
                    task_id,
                    UnderwaterSceneGenerationExperiment._generate_single_scene,
                    kwargs,
                )
            )
