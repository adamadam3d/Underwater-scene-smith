...
from reefsmith.agent_utils.support_surface_filters import (
...
def _convert_hsm_to_reefsmith_coords(points: np.ndarray) -> np.ndarray:
    """Convert coordinates from HSM (Y-up, Z-forward) to reefsmith (Z-up, Y-forward).
    ...
    Transformation: [X, Y, Z]_HSM → [X, Z, -Y]_reefsmith

    Args:
        points: Array of shape (N, 3) in HSM coordinates (Y-up, Z-forward).

    Returns:
        Array of shape (N, 3) in reefsmith coordinates (Z-up, Y-forward).
    """
    # HSM: X-right, Y-up, Z-forward
    # reefsmith/Drake: X-right, Y-forward, Z-up
    # Transform: [X, Y, Z]_HSM → [X, -Z, Y]_reefsmith
    # - X stays X (right direction in both)
    # - Y (up in HSM) becomes Z (up in reefsmith)
    # - Z (forward in HSM) becomes Y (forward in reefsmith), but negated because
    #   HSM Z-forward points in opposite direction to reefsmith Y-forward
...
def _corners_to_bbox_and_transform(
    ...
    # Apply clearance to Z dimension (vertical in reefsmith coords).
...
def load_hssd_support_surfaces(
    ...
    Returns:
        List of SupportSurface objects in reefsmith coordinates (Z-up, Y-forward,
    ...
    try:
        ...
        # Convert to reefsmith coordinates and create SupportSurface objects.
        ...
        for hsm_surface in hsm_surfaces:
            ...
            # Convert corners from HSM to reefsmith coordinates.
            corners_reefsmith = _convert_hsm_to_reefsmith_coords(hsm_surface.corners)
