...
1. **Floor Plan Agent $\rightarrow$ Seabed Agent:**
   - Transform `scenesmith/floor_plan_agents/` to `reefsmith/seabed_agents/`.
...
2. **Furniture Agent $\rightarrow$ Reef Agent:**
   - Transform `scenesmith/furniture_agents/` to `reefsmith/reef_agents/`.
...
3. **Wall/Ceiling Agents $\rightarrow$ Accent Agents:**
   - Transform `scenesmith/wall_agents/` and `scenesmith/ceiling_agents/` to `reefsmith/accent_agents/`.
...
4. **Manipuland Agent $\rightarrow$ Micro-Habitat Agent:**
   - Transform `scenesmith/manipuland_agents/` to `reefsmith/micro_habitat_agents/`.
...
The system relies on YAML files in `scenesmith/prompts/` to instruct the LLMs and VLMs. Update these to reflect the underwater domain.
...
1. **Hydra Configurations:** Update the pipeline orchestration in `configurations/experiment/`. Rename `indoor_scene_generation.yaml` to `underwater_scene_generation.yaml` and update the agent sequence to: `Seabed -> Reef -> Accent -> Micro-Habitat`.
2. **Robot Evaluation:** Update `scenesmith/robot_eval/` (which becomes `reefsmith/robot_eval/`) to validate marine robot (ROV/AUV) policies instead of terrestrial manipulation, changing predicates from things like "place item on table" to "inspect coral structure".
