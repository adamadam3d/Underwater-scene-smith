<h1 align="center">
  <img src="media/symbol.svg" alt="" height="36" align="absmiddle">&nbsp;ReefSmith
</h1>

<p align="center">
Code for "ReefSmith: Agentic Generation of Simulation-Ready Underwater Coral Reefs".
</p>

<p align="center">
  <a href="https://arxiv.org/abs/2602.09153"><img src="https://img.shields.io/badge/arXiv-2602.09153-b31b1b.svg" alt="arXiv"></a>
  <a href="https://reefsmith.github.io/"><img src="https://img.shields.io/badge/Project-Website-green.svg" alt="Project Website"></a>
...
<p align="center">
  <img src="media/teaser.webp" alt="ReefSmith teaser" width="100%">
</p>

<p align="center"><em>
<b>Fully automated text-to-reef generation.</b> This entire coral reef environment was generated by ReefSmith without any human intervention, from a single text prompt. Beyond explicitly specified elements, ReefSmith places additional marine life from inferred ecological context, such as anemones and urchins nestled into crevices near a brain coral. Assets are generated on-demand, are fully separable (non-composite), and include estimated physical properties (mass, drag, buoyancy), enabling direct interaction within a simulation. The resulting underwater scenes are immediately usable in arbitrary physics simulators (an ROV added for demonstration).
</em></p>
...
```bibtex
@misc{reefsmith2026,
  title={ReefSmith: Agentic Generation of Simulation-Ready Underwater Coral Reefs},
...
ReefSmith supports two backends for open-set 3D asset generation. If using
...
Alternatively, run ReefSmith in a Docker container with NVIDIA GPU support.
...
docker compose run --rm reefsmith bash
...
docker compose run --rm reefsmith python main.py +name=my_experiment

# Smoke test
docker compose run --rm reefsmith \
    python -c "import torch; print(torch.cuda.is_available()); import reefsmith"

# Unit tests
docker compose run --rm reefsmith pytest tests/unit/ -x
```
...
ReefSmith supports multiple asset and material sources. The **default configuration**
...
huggingface-cli download nepfaff/reefsmith-preprocessed-data \
...
2. **Download pre-computed CLIP embeddings** (recommended):
   ...
   huggingface-cli download nepfaff/reefsmith-preprocessed-data \
       ...
       --local-dir data/reefsmith-preprocessed-data
   mv data/reefsmith-preprocessed-data/ambientcg/embeddings data/materials/embeddings
   rm -rf data/reefsmith-preprocessed-data
   ```
...
ReefSmith's native output format is Drake Directives (`.dmd.yaml`).
...
<p align="center"><em>
<b>Robot manipulation evaluation pipeline.</b> Given a manipulation task (e.g., "Pick a fruit from the fruit bowl and place it on a plate"), an LLM generates diverse scene prompts specifying scene constraints implied by the task. ReefSmith generates scenes from each prompt. A robot policy attempts the task in simulation, and an evaluation agent verifies success using simulator state queries and visual observations. This enables scalable policy evaluation without manual environment or success predicate design.
</em></p>
...
2. **Generate Scenes** - Use `main.py` with generated prompts (ReefSmith pipeline)
